...
ChromeExOAuth.prototype.getRequestToken = function(callback, opt_args) {
  ...
  var ppp = {
    ...
    parameters: {
      ...
      "privileges": "DOC_CREATE+DOC_SEND+DOC_CHECK"
    },
    signatures: {
      consumer_key : this.consumer_key,
      shared_secret : this.consumer_secret
  ...
  var result = oauth_simple.sign(ppp);

  var onToken = ChromeExOAuth.bind(this.onRequestToken, this, callback);
  var headerNoQuotes = result.header; //.replace(/[\"\']/g,"");
  var headerNoQuotesWithCommas = headerNoQuotes.replace(/ /g,",");
  var headerNoQuotesWithCommasNoOAuth = headerNoQuotesWithCommas.substring(6);
  var headers = { "Authorization": headerNoQuotesWithCommasNoOAuth };
  ChromeExOAuth.sendRequest("GET", result.signed_url, headers, null, onToken);
};
